"""
Residual MLP for passivity-based WBC correction.

Three modes:
  --mode collect   Re-runs passivity controller on seeds 0-199, logs per-timestep
                   (state, tau_base) to residual_mlp_dataset.npz
  --mode train     Loads dataset, trains 42->64->64->64->6 network, saves weights
...
# MLP dimensions (locked design spec)
INPUT_DIM  = 42   # pitch, roll, d_pitch, d_roll (4) + q[19] + dq[19]
OUTPUT_DIM = 6    # Δτ for: L_ankle, R_ankle, L_hip_pitch, R_hip_pitch, L_hip_roll, R_hip_roll
HIDDEN_DIM = 64
TAU_BOUND  = 20.0  # ±20 Nm output clip
...
# =============================================================================
# SHARED UTILITIES  (duplicated from wbc_passivity_experiment.py to keep this
# file self-contained — same logic, same sign conventions)
# =============================================================================
...
class ResidualMLP(nn.Module):
    """42 → 64 → 64 → 64 → 6, ReLU hidden, tanh × TAU_BOUND output."""
...
class PassivityEngine:
    ...
    # ── Passivity torque (identical to wbc_passivity_experiment.py) ───────────
    ...
    def _build_state_vec(
        ...
        """Build 42-dim MLP input vector. Same order as design spec."""
        d_pitch = (curr_pitch - prev_pitch) / dt_est
        d_roll  = (curr_roll  - prev_roll)  / dt_est
        q       = self.data.qpos[self.qaddr]   # shape (19,)
        dq      = self.data.qvel[self.daddr]   # shape (19,)
        vec = np.concatenate([
            [curr_pitch, curr_roll, d_pitch, d_roll],  # 4
            q,                                          # 19
            dq,                                         # 19
        ])                                              # total = 42
    ...
    def run_trial(
        ...
        stable_only_collect: bool = True,   # collect timesteps only from stable trials
    ) -> Dict[str, Any]:
        """
        Run one trial.
        - collect_data=True  → accumulate (state, tau) pairs into self._collect_*
        - mlp_model not None → apply MLP correction on top of base torque
        """
        ...
        # Per-trial data buffer (only flushed to self._collect_* if stable)
        ...
        def step_once() -> None:
            ...
            if mlp_model is not None and now_t >= first_push_start:
                state_vec = self._build_state_vec(
                    curr_pitch, curr_roll, prev_pitch, prev_roll, dt_est
                )
                with torch.no_grad():
            ...
            # Collect: log (state, tau_base) — before MLP so MLP learns residual
            if collect_data and now_t >= first_push_start:
                state_vec = self._build_state_vec(
                    curr_pitch, curr_roll, prev_pitch, prev_roll, dt_est
                )
                tau_out = tau_wbc[self.mlp_output_indices].copy()
...
def run_collect(args: argparse.Namespace) -> None:
    """Re-run passivity controller, log per-timestep (state, tau) for stable seeds."""
    ...
    print(f"Stable-only logging: {not args.collect_all}")
    print("=" * 60)
    ...
    for seed in range(args.n_collect_seeds):
        ...
        res = engine.run_trial(
            profile,
            collect_data=True,
            stable_only_collect=not args.collect_all,
        )
    ...
    states_arr = np.array(engine._collect_states, dtype=np.float32)  # (N, 42)
    taus_arr   = np.array(engine._collect_taus,   dtype=np.float32)  # (N, 6)

    # Per-channel statistics for normalisation (saved alongside weights)
...
def lipm_prediction_loss(
    ...
    The LIPM predicts CoM_ddot = (g/z_c) * (CoM - CoP).
    We use pitch as a proxy for sagittal CoM error and the ankle delta_tau
    as a proxy for CoP correction: delta_CoP ≈ delta_tau_ankle / (m * g).

    Loss = || predicted_d_pitch_next - actual_d_pitch_next ||^2
    where:
      predicted_d_pitch_next = d_pitch + (g/z_c) * pitch * dt
                               - (1/(m*z_c)) * delta_tau_ankle * dt

    This penalises MLP corrections that would destabilise the LIPM.
    """
    pitch   = states[:, 0]      # index 0
    d_pitch = states[:, 2]      # index 2

    # ankle delta torques: OUTPUT_JOINT_NAMES[0]=L_ankle, [1]=R_ankle
    delta_ankle = (delta_tau[:, 0] + delta_tau[:, 1]) * 0.5  # mean ankle correction

    k = float(g / z_c)
    predicted_d_pitch_next = d_pitch + k * pitch * dt - (1.0 / (m * z_c)) * delta_ankle * dt

    # We want d_pitch to decrease toward zero — penalise growing pitch rate
...
def run_train(args: argparse.Namespace) -> None:
    if not TORCH_AVAILABLE:
        raise RuntimeError("PyTorch not available. Run: pip install torch --break-system-packages")

    dataset_path = os.path.join(args.output_dir, args.dataset_file)
    if not os.path.exists(dataset_path):
        raise FileNotFoundError(
            f"Dataset not found: {dataset_path}\n"
            f"Run --mode collect first."
        )
    ...
    states_all = data_np["states"]   # (N, 42)  float32
    taus_all   = data_np["taus"]     # (N, 6)   float32
    state_mean = data_np["state_mean"]
    state_std  = data_np["state_std"]
    tau_mean   = data_np["tau_mean"]
    tau_std    = data_np["tau_std"]

    print(f"  Total timesteps : {len(states_all):,}")
    print(f"  State shape     : {states_all.shape}")
    print(f"  Tau shape       : {taus_all.shape}")

    # Normalise inputs; targets are raw torques (the MLP predicts residuals in Nm)
    states_norm = (states_all - state_mean) / state_std

    # Train / val split by approximate proportion matching the seed split
    # (we don't have per-seed boundaries in the flat array, so we split by index)
    n_total = len(states_norm)
    n_train = int(n_total * (len(TRAIN_SEEDS) / (len(TRAIN_SEEDS) + len(VAL_SEEDS))))
    # Shuffle before split (seed fixed for reproducibility)
    ...
    y_train = torch.zeros(len(train_idx), OUTPUT_DIM, dtype=torch.float32)
    X_val   = torch.tensor(states_norm[val_idx],   dtype=torch.float32)
    y_val   = torch.zeros(len(val_idx),   OUTPUT_DIM, dtype=torch.float32)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for X_b, y_b in train_dl:
            ...
            optimizer.zero_grad()
            pred       = model(X_b)
            # Reconstruct unnormalised states for LIPM loss (approximate: use raw batch)
            # For LIPM loss we need pitch/d_pitch in real units, so un-normalise first 4 dims
            ...
            X_raw        = X_b * state_std_t + state_mean_t

            

            reg_loss  = reg_lambda * (pred ** 2).mean()
            lipm_loss = lipm_prediction_loss(X_raw, pred)
            loss      = reg_loss + lipm_loss
...
def run_evaluate(args: argparse.Namespace) -> None:
    ...
    print(f"  Best val loss from training: {checkpoint.get('best_val_loss', 'N/A')}")

    # Patch args so controller_name appears as "passivity_mlp" in CSV
    args.controller_name = "passivity_mlp"

    engine = PassivityEngine(args)

    # Wrap model so engine can normalise inputs before inference
    ...
    csv_path   = os.path.join(
        args.output_dir, f"passivity_mlp_random_{session_id}.csv"
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Residual MLP for passivity-based WBC")

    p.add_argument("--mode", type=str, required=True,
                   choices=["collect", "train", "evaluate"],
                   help="collect | train | evaluate")
    ...
    p.add_argument("--dataset-file",  type=str, default="residual_mlp_dataset.npz")
    p.add_argument("--weights-file",  type=str, default="residual_mlp_weights.pt")
    p.add_argument("--controller-name", type=str, default="passivity_based")

    # Simulation parameters (must match passivity script exactly)
    ...
    p.add_argument("--init-noise",         type=float, default=0.0,
                   help="Set to 0 for collect/evaluate to get clean deterministic reruns.")

    # Collect options
    p.add_argument("--n-collect-seeds", type=int, default=200,
                   help="Number of seeds to collect from (collect mode).")
    p.add_argument("--collect-all",     action="store_true",
                   help="If set, collect timesteps from ALL trials (not just stable ones).")
    ...
    p.add_argument("--n-eval-seeds", type=int, default=200,
                   help="Number of seeds to evaluate (evaluate mode).")
