"""
Automated humanoid disturbance experiments for Hierarchical QP controller.
...
class HumanoidExperimentEngine:
    ...
    def _solve_hqp(self, tau_nom: np.ndarray) -> Tuple[np.ndarray, float, str, str]:
        """Solve the two-stage HQP. Returns (tau_wbc, solve_ms, bal_status, post_status).
        Shared by both run_single_trial and run_random_trial to avoid duplication.
        """
        ...
        try:
            ...
            tau_wbc = tau_balance
            posture_status = "err"
        solve_ms = (time.perf_counter() - _t0) * 1000.0
        return tau_wbc, solve_ms, balance_status, posture_status
    ...
    def run_single_trial(self, config: Dict[str, Any], trial_in_condition: int, visualize: bool) -> Dict[str, Any]:
        ...
        # v8 lesson: initialise prev_pitch from actual state, not 0.0
        _, _pitch0 = get_rpy(self.data.qpos[3:7])
        _roll0, _ = get_rpy(self.data.qpos[3:7])
        prev_pitch = float(_pitch0)
        self._prev_roll = float(_roll0)
        ...
        def step_once() -> None:
            ...
            # Input validation (DPP / NaN guards).
            ...
            tau_wbc, solve_ms, balance_status, posture_status = self._solve_hqp(tau_nom)
            qp_solve_times.append(float(solve_ms))

            ctrl = np.clip(tau_wbc / self.gear, self.ctrl_min, self.ctrl_max)
            self.data.ctrl[:] = ctrl

            # Rate-limited debug print (only in debug mode).
            if int(getattr(self.args, "hqp_debug", 0)) == 1 and str(getattr(self.args, "mode", "")) == "debug":
                ...
                if now_t <= dbg_until and (now_t - dbg_last_print_t) >= dbg_dt:
                    ...
                    print(
                        ...
                        f"tau|max nom={float(np.max(np.abs(tau_nom))):.1f} wbc={float(np.max(np.abs(tau_wbc))):.1f} "
        ...
        result = {
            ...
            # Part 1b columns — sentinels for Part 1a trials (schema-consistent with random mode).
            "disturbance_seed": -1,
    ...
    def run_random_trial(self, profile: "DisturbanceProfile", visualize: bool = False) -> Dict[str, Any]:
        ...
        # Part 1b new metrics: integrated CoM² and full-trial torque².
        com_sq_integral = 0.0
        ...
        def step_once() -> None:
            ...
            tau_wbc, solve_ms, _bs, _ps = self._solve_hqp(tau_nom)
            qp_solve_times.append(float(solve_ms))

            ctrl = np.clip(tau_wbc / self.gear, self.ctrl_min, self.ctrl_max)
            self.data.ctrl[:] = ctrl
            # NOTE: no per-step debug print in random mode (200× noise suppression).
            ...
            # Apply scheduled push(es) — clear previous, then look up active push.
            self.data.xfrc_applied[:] = 0.0
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--controller-name", type=str, default="hierarchical_qp")
