...
try:
    from wbc_hierarchial_qp_experiment_final import HumanoidExperimentEngine as HQPEngine
...
class HQPEngineWrapper:
    def __init__(self, args: argparse.Namespace):
        if HQPEngine is None:
            raise RuntimeError(f"Failed to import HQPEngine: {_HQP_IMPORT_ERROR}")
        if cp is None:
            raise RuntimeError("cvxpy is required to run HQP residual MLP (collect/evaluate).")
        self.args = args
        self.engine = HQPEngine(args)
    ...
    def _compute_hqp_torque(
        ...
        try:
            tau_wbc, _solve_ms, _bal_status, _post_status = self.engine._solve_hqp(tau_nom)
        except Exception:
            tau_wbc = tau_nom
    ...
    def run_trial(
        ...
        while self.data.time < sim_end:
            ...
            tau_wbc, hipr_int = self._compute_hqp_torque(curr_pitch, curr_roll, prev_pitch, prev_roll, dt_est, hipr_int)
...
def run_collect(args: argparse.Namespace) -> None:
    ...
    print("=" * 60)
    print("COLLECT MODE  (HQP residual MLP v3)")
...
def run_train(args: argparse.Namespace) -> None:
    ...
    print("=" * 60)
    print("TRAIN MODE  (HQP residual MLP v3)")
...
def run_evaluate(args: argparse.Namespace) -> None:
    ...
    print("=" * 60)
    print("EVALUATE MODE  (HQP residual MLP v3)")
    ...
    args.controller_name = "hqp_mlp_v3"
    ...
    session_id = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    os.makedirs(args.output_dir, exist_ok=True)
    csv_path = os.path.join(args.output_dir, f"hqp_mlp_v3_random_{session_id}.csv")
    ...
    print("\n" + "=" * 60)
    print("EVALUATE COMPLETE  (hqp_mlp_v3)")
    print(f"  Stable          : {n_stable}/{n}  ({100*n_stable/n:.1f}%)")
    print("  HQP baseline    : 103/200 (51.5%)  ← compare against this")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Residual MLP v3 applied to Hierarchical QP controller")
    ...
    p.add_argument("--dataset-file", type=str, default="residual_mlp_dataset_hqp.npz")
    p.add_argument("--weights-file", type=str, default="residual_mlp_weights_hqp.pt")
